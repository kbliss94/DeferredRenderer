...
namespace TerrainRenderer
{
	...
	*/
	...
	void QuadTree::CreateTreeNode(NodeType* node, float positionX, float positionZ, float width, ID3D11Device* device)
	{
		...
		for (int i = 0; i < mTriangleCount; i++)
		{
			...
			if (result == true)
			{
				...
				node->vertexArray[index].x = mVertexList[vertexIndex].position.x;
				node->vertexArray[index].y = mVertexList[vertexIndex].position.y;
				node->vertexArray[index].z = mVertexList[vertexIndex].position.z;
	...
	bool QuadTree::GetHeightAtPosition(float posX, float posZ, float& height)
	{
		...
		meshMinX = mParentNode->positionX - (mParentNode->width / 2.0f);
		meshMaxX = mParentNode->positionX + (mParentNode->width / 2.0f);

		meshMinZ = mParentNode->positionZ - (mParentNode->width / 2.0f);
		meshMaxZ = mParentNode->positionZ + (mParentNode->width / 2.0f);

		//making sure the coordinates are actually over a polygon
		if ((posX < meshMinX) || (posX > meshMaxX) || (posZ < meshMinZ) || (posZ > meshMaxZ))
		{
			return false;
		}

		//finding the node which contains the polygon for this position
		FindNode(mParentNode, posX, posZ, height);
