...
namespace TerrainRenderer
{
	...
	bool TerrainManager::GetHeightAtPosition(float posX, float posZ, float& height)
	{
		if (CLIMBING_ENABLED && QUADTREES_ENABLED)
		{
			for (int i = 0; i < mNumGridRows; ++i)
			{
				if (mQuadTopRow[i]->GetHeightAtPosition(posX, posZ, height))
				{
					return true;
				}
				else if (mQuadMiddleRow[i]->GetHeightAtPosition(posX, posZ, height))
				{
					return true;
				}
				else if (mQuadBottomRow[i]->GetHeightAtPosition(posX, posZ, height))
				{
					return true;
				}
			}
