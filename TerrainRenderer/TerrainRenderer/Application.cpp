...
namespace TerrainRenderer
{
	...
	bool Application::Frame()
	{
		...
		// Handling height adjustment for camera
		if (CLIMBING_ENABLED && QUADTREES_ENABLED)
		{
			D3DXVECTOR3 position = mCamera->GetPosition();
			float height;

			//getting the height of the triangle that is directly underneath the given position
			if (mTerrainManager->GetHeightAtPosition(position.x, position.z, height))
			{
				//positioning the camera above the terrain 
				mCamera->SetPosition(position.x, height + 10.0f, position.z);
			}
		}
