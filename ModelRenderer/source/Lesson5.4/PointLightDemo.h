...
namespace Rendering
{
	...
	private:
		struct MeshData
		{
			...
			MeshData() = default;
			MeshData(Microsoft::WRL::ComPtr<ID3D11Buffer> vertexBuffer, Microsoft::WRL::ComPtr<ID3D11Buffer> indexBuffer, std::uint32_t indexCount) :
				VertexBuffer(vertexBuffer), IndexBuffer(indexBuffer), IndexCount(indexCount), ColorTexture() {}
		};
		...
		//contains the vector buffer, index buffer, color texture, and index count for each mesh
